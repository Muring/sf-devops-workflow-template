# GitHub Actions 기반 Salesforce CI/CD (main < develop < dev < feature) 운영 가이드

> 본 문서는 저장소에 포함된 4개의 GitHub Actions Workflow 파일을 기준으로, **브랜치 전략(main < develop < dev < feature)** 과 그에 따른 **자동 검증(Validate) + Quick Deploy + Dev Org 직접 배포 + (Production) Release 생성** 흐름을 상세히 설명합니다.
> 대상 파일:
...
- [2. 브랜치 전략: main &lt; develop &lt; dev &lt; feature](#2-브랜치-전략-main--develop--dev--feature)
- [3. Workflow 전체 개요(4종)](#3-workflow-전체-개요4종)
...
- [8. Dev Org Deploy 상세 (deploy-devyml)](#8-dev-org-deploy-상세-deploy-devyml)
- [9. “ValidatedDeployId 브릿지” 설계 의도](#9-validateddeployid-브릿지-설계-의도)
- [10. 운영 규칙/권장사항](#10-운영-규칙권장사항)
- [11. 자주 발생하는 이슈와 트러블슈팅](#11-자주-발생하는-이슈와-트러블슈팅)
- [12. 파일별 “코드 레벨” 해설(스텝 단위)](#12-파일별-코드-레벨-해설스텝-단위)
- [13. 결론: 이 구조의 장점](#13-결론-이-구조의-장점)
- [부록 A. PR 본문 템플릿 예시](#부록-a-pr-본문-템플릿-예시)
- [부록 B. 파일 원문](#부록-b-파일-원문)
- [14. 핵심 코드 스니펫 상세 해설(실제 YAML 발췌 + 설명)](#14-핵심-코드-스니펫-상세-해설실제-yaml-발췌--설명)
...
## 2. 브랜치 전략: main < develop < dev < feature

본 저장소는 다음과 같은 브랜치 역할을 전제로 합니다.

- **feature/***: 개발자가 기능/수정 단위로 작업하는 브랜치
- **dev**: 개발자 개인/팀 Dev Org 배포 브랜치. feature 브랜치에서 PR 또는 직접 push
- **develop**: 통합 브랜치(샌드박스 대상). dev/feature 브랜치들이 PR로 들어오는 곳
- **main**: 프로덕션 브랜치(운영 대상). develop에서 검증된 변경만 PR로 들어오는 곳

### 권장 운영 흐름(가이드)

1. 개발자는 `feature/*` 브랜치를 생성하여 작업
2. `feature/*` → `dev` 로 Push 또는 Pull Request 생성
3. `dev` 브랜치 push/PR 시, **Dev Org Deploy Workflow**가 자동 실행되어 Dev Org에 Delta 직접 배포(`NoTestRun`)
4. 개발이 완료되면 `feature/*` → `develop` 로 Pull Request 생성
5. PR 생성 시, **PR Validate Workflow**가 자동 실행되어 Sandbox(또는 Validate 전용 Org)에 대해 배포 검증
6. 검증이 통과하면 PR 코멘트에 `ValidatedDeployId`가 남음
7. PR을 merge하여 `develop`에 반영하면, `develop push` 트리거로 **Sandbox Quick Deploy Workflow**가 실행되어 빠르게 배포
8. `develop` → `main` PR을 생성하면, 동일하게 PR Validate가 Production(또는 Production Validate Org)에 대해 검증
9. PR merge로 `main`에 반영되면, `main push` 트리거로 **Production Quick Deploy + Release Workflow**가 실행되어 배포 및 릴리즈 산출물(태그/릴리즈)이 생성
...
### 3.4 Dev Org Deploy (dev 브랜치 push/PR)

- 파일: `deploy-dev.yml`
- 트리거: `push` (브랜치: `dev`) / `pull_request` (대상 브랜치: `dev`)
- 역할:
  - push/PR 이벤트 모두 처리, SHA 기반 Delta 추출
  - Validate 없이 `sf project deploy start`(`NoTestRun`)로 Dev Org에 직접 배포
  - PR 이벤트 시 배포 결과(`DevOrgDeployId`, Status)를 PR 코멘트로 기록
  - 신규 브랜치 생성 시(null SHA) 자동 스킵 처리
...
## 8. Dev Org Deploy 상세 (`deploy-dev.yml`)

Dev Org Deploy는 개발 중인 변경을 빠르게 Dev Org에 반영하기 위한 Workflow입니다.
Validate 없이 직접 배포하므로, **개발/테스트 용도 전용**이며 Sandbox/Production 흐름과는 독립적입니다.
...
concurrency:
  group: deploy-dev-${{ github.event_name }}-${{ github.event.pull_request.number || github.ref }}
  cancel-in-progress: true
```

- push와 pull_request 이벤트 모두 처리
- PR 번호 또는 ref 기준으로 동시성 그룹을 구성하여, 같은 브랜치/PR에 대한 중복 실행을 취소
- `cancel-in-progress: true`: 빠른 피드백이 목적이므로 이전 실행을 취소하고 최신만 유지

### 8.2 SHA 해석 및 신규 브랜치 스킵
...
if [ “$FROM_SHA” = “$NULL_SHA” ] || [ -z “$FROM_SHA” ]; then
  echo “SKIP=true” >> “$GITHUB_OUTPUT”
fi
```

- PR 이벤트: base/head SHA를 사용하여 PR 단위 변경분 추출
- push 이벤트: `before`/`sha`를 사용하여 해당 push에서 달라진 변경분만 추출
- `before`가 null SHA(브랜치 최초 생성)이면 배포 없이 스킵 처리

### 8.3 Delta 빌드

PR Validate와 동일한 방식으로 `sfdx-git-delta`로 변경분을 추출합니다.
manifest(`package.xml`)와 destructiveChanges를 생성하며, 변경이 없으면 배포를 건너뜁니다.

### 8.4 Dev Org에 직접 배포 (`NoTestRun`)
...
- `sf project deploy start`: validate 없이 바로 배포 (Quick Deploy가 아닌 일반 배포)
- `--test-level NoTestRun`: 개발 환경이므로 테스트 실행 없이 빠르게 배포
- destructive 변경이 있으면 `--post-destructive-changes` 추가

### 8.5 PR 코멘트로 배포 결과 기록

PR 이벤트 시 배포 결과를 PR 코멘트로 남깁니다.

```
DevOrgDeployId: 0AfXXXXXXXXXXXX
Status: Succeeded
HeadSHA: abcdef123456...
Note: Direct deploy (NoTestRun) to dev org.
```

- 변경이 없거나 브랜치가 새로 생성된 경우에는 SKIPPED 메시지를 코멘트로 남김
- push 이벤트(merge 없는 직접 push)는 코멘트 없이 배포만 수행

---

## 9. “ValidatedDeployId 브릿지” 설계 의도
...
### 10.4 여러 packageDirectories(모노레포) 사용 시
...
## 11. 자주 발생하는 이슈와 트러블슈팅

### 11.1 “No PR found for commit” 에러
...
### 11.2 ValidatedDeployId가 PR 코멘트에 없음
...
### 11.3 Quick Deploy가 실패(Status가 Succeeded가 아님)
...
### 12.2 Sandbox Quick Deploy Step 요약
...
### 12.3 Production Quick Deploy + Release Step 요약
...
### 12.4 Dev Org Deploy Step 요약

- Resolve SHA: push/PR 이벤트별 FROM/TO SHA 결정, null SHA 신규 브랜치 스킵
- Build delta: `sfdx-git-delta`로 변경분 추출
- Guard + Auth: 필수 Secret 확인, JWT 로그인
- Deploy delta: `NoTestRun`으로 Dev Org 직접 배포
- Comment: PR 이벤트 시 `DevOrgDeployId` + Status를 PR 코멘트로 기록
...
- **개발 편의**: Dev Org 직접 배포로 빠른 개발 피드백 루프 지원
...
> 저장소에 `.github/pull_request_template.md` 파일이 포함되어 있어, PR 생성 시 아래 형식이 자동으로 채워집니다.
...
### 14.2 변경분만 추출하는 Delta 빌드(성능/안정성의 핵심)
...
### 14.3 PR 본문에서 테스트 전략을 “규칙”으로 뽑아내는 방식
...
### 14.4 Validate 실행 시, 테스트 레벨/Destructive 옵션을 동적으로 조립
...
### 14.5 Validate Job Id 추출 → PR 코멘트로 저장(Quick Deploy 브릿지)
...
### 14.6 merge 후 push에서 “커밋 SHA → PR 번호”를 역으로 찾는 로직
...
### 14.7 PR 코멘트에서 최신 ValidatedDeployId 추출(여러 번 검증된 경우 대비)
...
### 14.8 Production 배포 후 릴리즈 태그 자동 증가 로직
